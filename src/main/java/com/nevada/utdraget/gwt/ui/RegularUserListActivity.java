 ...
 * records.
 */
public class RegularUserListActivity extends
    AbstractRecordListActivity<RegularUserRecord> {
   ...
   */
  public RegularUserListActivity(HandlerManager eventBus,
   ...
   */
  public RegularUserListActivity(HandlerManager eventBus,
      ApplicationRequestFactory requests, RecordListView<RegularUserRecord> view,
      PlaceController<ApplicationPlace> placeController) {
    super(view);
  ...
  public void start(Display display) {
    this.registration = eventBus.addHandler(RegularUserRecordChanged.TYPE,
        new RegularUserChangedHandler() {
  ...
  protected RecordListRequest<RegularUserRecord> createRangeRequest(Range range) {
    return requests.regularUserRequest().findRegularUserEntries(range.getStart(),
        range.getLength());
  }
